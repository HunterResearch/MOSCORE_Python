...
def score_allocation(systems, warm_start=None):
    ...
    def constraint_values(x):
        return score_constraints_wrapper(x, systems, phantoms, num_par, m_star, j_star, lambdas, n_obj, n_systems)[0]

    def constraint_jacobian(x):
        score_constraints_wrapper(x, systems, phantoms, num_par, m_star, j_star, lambdas, n_obj, n_systems)[1]
    ...
    # TODO: See if this block is needed.
...
def MCE_score_rates(alphas, systems, M_star, n_obj, n_systems):
    ...
    for k in range(n_MCE):
        ...
        if (alphas[i] < tol or alphas[j] < tol):
            ...
            if n_obj == 2:
                rate, grad_i, grad_j = MCE_2d(aI=alphas[i],
                                              aJ=alphas[j],
                                              Iobj=obj_i,
                                              Isig=systems["var"][i_ind],
                                              Jobj=obj_j,
                                              Jsig=systems["var"][j_ind],
                                              inv_var_i=inv_cov_i,
                                              inv_var_j=inv_cov_j
                                              )
            elif n_obj == 3:
                rate, grad_i, grad_j = MCE_3d(aI=alphas[i],
                                              aJ=alphas[j],
                                              Iobj=obj_i,
                                              Isig=systems["var"][i_ind],
                                              Jobj=obj_j,
                ...
                rate, grad_i, grad_j = MCE_four_d_plus(alpha_i=alphas[i],
                                                       alpha_j=alphas[j],
                                                       obj_i=obj_i,
                                                       inv_var_i=inv_cov_i,
                                                       obj_j=obj_j,
