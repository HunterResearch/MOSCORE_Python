...
% check to see whether Octave or Matlab is being run
if isOctave()
  EqualCons = @(alphas) calcEqCons(alphas,num_par);
  ObjFunOct = @(alphas) calcobjOuterOct(alphas,scaling,num_par);
  ObjFunGrad = @(alphas) calcobjGradOct(alphas,scaling,num_par);
  OctConstraints = @(alphas) OctCons(alphas,EstObj,Phantoms,num_par,Mstar,Jstar,Lambdas);
  %InEqualCons = -1*Constraints(1);
  %InEqualGrads = @(alphas) -1*Constraints(3);
  % first pass, find solution when StepTol = 1e-8

  %[x, obj, info, iter, nf, lambda] = sqp(a0, {ObjFunOct, ObjFunGrad}, EqualCons, {InEqualCons,InEqualGrads}, lower_bound, upper_bound, settingsSeq.fmincon.MaxIter, settingsSeq.fmincon.TolX);
  [a1, fval, info, iter, nf, lambda] = sqp(a0, ObjFunOct, EqualCons, OctConstraints, lower_bound, upper_bound, 5000, 1e-18)
else
  % Run Matlab Fmincon
...
if isOctave()==0  % do iterations in Matlab
...
function objvalOuter = calcobjOuterOct(alphas,c,num_par)
        objvalOuter = -c*alphas(num_par+2);
end

function objGrad = calcobjGradOct(alphas,c,num_par)
        objGrad = [zeros(num_par+1,1);-c];
end

function EqCons = calcEqCons(alphas,num_par)
        alphAeq = [ones(1,num_par+1),0];
        alphbeq = 1.0;
        EqCons = alphAeq*alphas - alphbeq;
end

function OctConstraints = OctCons(alphas,EstObj,Phantoms,num_par,Mstar,Jstar,Lambdas)
        OctConstraints = Prod_Rates_iS_OCT(alphas,EstObj,Phantoms,num_par,Mstar,Jstar,Lambdas);
end
